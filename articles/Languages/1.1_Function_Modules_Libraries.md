...
A module is a single Python file with a `.py` extension. It contains functions and other code. When you import a module, you bring in all the functionality defined in that file.
...
- **Import the Whole Module:** `import module_name`
  ...
  import mymodule
  ```
  This imports the entire module, and you can access its functions with the `module_name.` prefix.
  
- **Import Specific Functions:** `from module_name import function_name`
  ```python
  from mymodule import add
  ```
  This imports only the specified function(s) from the module, allowing you to use them directly without the `module_name.` prefix.

**Standard Modules**
math, os, json, datetime

### `import module` vs `from module import *`

- **`import module`**:
  - **Imports the entire module**: This means the module itself is imported as a whole.
  - **Access**: You need to prefix any function, class, or variable from the module with the module name.
  - **Example**:
    ```python
    import math
...
- **`from module import *`**:
  - **Imports all functions, classes, and variables from the module directly into the current namespace**: This means each function, class, and variable in the module is individually imported into your current namespace.
  ...
  - **Example**:
    ...
    ```
  - **Drawbacks**: Can create naming conflicts if different modules have functions, classes, or variables with the same name. It can also make it harder to find where a function/class/variables etc came from, especially if you have a big programme or there are many modules.
