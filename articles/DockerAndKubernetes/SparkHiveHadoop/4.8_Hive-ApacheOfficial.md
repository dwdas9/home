...
- [Apache Official - Hive Metastore and HiveServer2 Setup On Docker](#apache-official---hive-metastore-and-hiveserver2-setup-on-docker)
  - [Overview](#overview)
...
- [How data is stored in this setup by default](#how-data-is-stored-in-this-setup-by-default)
- [Steps for Spark to Store Data in Hive](#steps-for-spark-to-store-data-in-hive)
    - [Key Points:](#key-points)
- [Connecting To Spark using `hive-site.xml`](#connecting-to-spark-using-hive-sitexml)
- [Configuration Reference](#configuration-reference)


# Apache Official - Hive Metastore and HiveServer2 Setup On Docker
...
In this setup, Hive data will be stored in the following places:
...
# Steps for Spark to Store Data in Hive
...
1. **Create a Spark Session**:
   ...
   spark = SparkSession.builder \
       ...
       .config("hive.metastore.uris", "thrift://hiveserver2:10000") \
       .enableHiveSupport() \
       .getOrCreate()
   ```

   - This configuration tells Spark to use Hive as the metadata catalog and points to the **Thrift URI** of HiveServer2 (`thrift://hiveserver2:10000`).
...
### Key Points:
- **No `hive-site.xml` Required**: By using the Thrift URI (`thrift://hiveserver2:10000`) in the Spark session, Spark can interact with Hive without needing `hive-site.xml`.
- **Data Location**: The actual table data is stored in the `warehouse` directory mounted to the Hive containers.
- **Metadata Access**: Spark accesses the table structure and metadata via the **HiveServer2** Thrift connection.

This setup allows a Spark application to seamlessly interact with Hive and store data in the Hive warehouse, without additional configurations.

# Connecting To Spark using `hive-site.xml`
